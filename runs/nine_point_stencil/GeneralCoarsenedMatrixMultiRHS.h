...
public:
  ...
  std::vector<deviceVector<ComplexF *> > BLAS_AP;
  std::vector<deviceVector<ComplexF *> > BLAS_BP;
  deviceVector<ComplexF *>               BLAS_CP;
  ...
  {
    // assert(A.size()==geom_srhs.npoint); // ME
    GridtoBLAS(A,BLAS_A[p]);
  }
  ...
  */
    ...
    Stencil(Cell.grids.back(),geom.shifts) // padded cell stencil
  {
	...
	ComplexF *ptr = (ComplexF *)&BLAS_A[p][ss];
	acceleratorPut(BLAS_AP[p][ss],ptr);
      }
    }
    // Site identity mapping for C
    for(int ss=0;ss<unpadded_sites;ss++){
      ComplexF *ptr = (ComplexF *)&BLAS_C[ss*nrhs];
	...
	for(int32_t point = 0 ; point < geom.npoint; point++){
	  ...
	  ComplexF * ptr = (ComplexF *)&BLAS_B[nbr];
...
//Grid : Message : 11698.730568 s : CoarsenOperator inv  103948313 us <-- Cut this ~10x if lucky by loop fusion
    ...
    for(int k=0;k<npoint;k++){ // Loop over momenta
      ...
      for(int l=0;l<npoint;l++){ // Loop over nbr relative
	ComplexF phase(0.0,0.0);
	for(int mu=0;mu<Nd;mu++){
	  RealF TwoPiL =  M_PI * 2.0/ clatt[mu];
	  phase=phase+TwoPiL*geom_srhs.shifts[k][mu]*geom_srhs.shifts[l][mu];
	}
	phase=exp(phase*ci);
	Mkl(k,l) = phase;
  ...
  void M (const CoarseVector &in, CoarseVector &out)
  {
    ...
    std::cout << "Coarse Mult exch " << t_exch << std::endl;
    std::cout << "Coarse Mult mult " << t_mult << std::endl;
    std::cout << "Coarse Mult GtoB " << t_GtoB << std::endl;
    std::cout << "Coarse Mult BtoG " << t_BtoG << std::endl;
    std::cout << "Coarse Mult tot  " << t_tot  << std::endl;
