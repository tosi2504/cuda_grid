...
class Plotter:
    ...
    def reduce(self):
        ...
        for target in targets:
            ...
            summed_times_resized = np.resize(
                summed_times, new_shape=(Plotter.data_len_reduced, Plotter.n_reps)
            )

            reduced_data_target["time"] = np.zeros(
                shape=(Plotter.data_len_reduced, Plotter.n_time_slices),
                dtype=np.float64,
            )
            times_reshaped = np.resize(
                self.data[target]["time"],
                new_shape=(
                    Plotter.data_len_reduced,
                    Plotter.n_reps,
    ...
    def plot_fixed_grid(self, grid: str = "8.8.8.8", normalization=None):
        fig, axs = plt.subplots(
            nrows=len(Plotter.Ns),
            ncols=len(Plotter.numRHSs),
            sharex=True,
            sharey=True,
            figsize=(12, 6),
        )
        ...
        for N, axs_fixed_N in zip(Plotter.Ns, axs):
            for numRHS, ax in zip(Plotter.numRHSs, axs_fixed_N):
                slice_data = np.zeros(
                    (Plotter.n_targets, Plotter.n_time_slices), dtype=np.float64
                )
                ...
                for i, slice_label in enumerate(Plotter.slice_labels):
                    ax.bar(
                        Plotter.targets, slice_data[i], label=slice_label, bottom=bottom
                    )
                    bottom += slice_data[i]
