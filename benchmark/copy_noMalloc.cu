...
void copy_no_malloc(bVectorField<T,N> * const * const ys
...
int main (int argc, char * argv[]) {
	...
	CCE(  cudaMalloc(&d_X, sizeof(T)*numRHS*grid.numSites*N)  );
	CCE(  cudaMalloc(&d_Y, sizeof(T)*numRHS*grid.numSites*N)  );
	auto func = copy_no_malloc<T,N,numRHS,blkSize>;
	BENCHMARK(resTime, 1000, func, ys, A, xs, d_Y, d_X);
	CCE(  cudaFree(d_X)  );
	CCE(  cudaFree(d_Y)  );

	// print out the results
	print_results<T>("copy_noMalloc", resTime, N, numRHS, blkSize, grid, 999, true);
}
